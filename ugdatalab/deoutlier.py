...
class MixtureContaminationModel:
    ...
    def _inlier_probs(flat_samples, x, y, sigma_y, mu_bg, sig_bg):
        ...
        """
        a, b, log10_sig, logit_f = flat_samples.T
        sig_scatter = (10.0**log10_sig)[:, None]
        f = (1.0 / (1.0 + np.exp(-logit_f)))[:, None]
        t_in, t_out = MixtureContaminationModel._log_mix_terms(
            a[:, None], b[:, None], sig_scatter, f, x[None, :], y[None, :], sigma_y[None, :], mu_bg, sig_bg
        )
    ...
    def _run_mcmc(x, y, sigma_y, n_walkers=32, n_steps=2000, seed=42):
        ...
        sampler = emcee.EnsembleSampler(
            n_walkers,
            4,
            MixtureContaminationModel._log_prob,
    ...
    def __init__(
        ...
        for label, mask in [
            ...
            probs = self._inlier_probs(flat, x[valid], y[valid], sig[valid], mu_bg, sig_bg)
