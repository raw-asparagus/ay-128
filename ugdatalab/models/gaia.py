...
def sanitize_vari_rrlyrae_table(data: table.Table) -> table.Table:
    ...
    out["num_clean_epochs_g"] = np.asarray(out["num_clean_epochs_g"], dtype=int)
    out["best_classification"] = np.asarray(out["best_classification"]).astype(str)
    for name in _RRLYRAE_FLOAT_COLUMNS:
