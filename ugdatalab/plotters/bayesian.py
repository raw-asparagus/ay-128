...
def plot_trace(result):
    ...
    h = 28 * (ndim + 1) / TEXTWIDTH_IN
    fig, ax = textwidth_figure(h)
    ax.remove()
    axes = subpanels(fig, ndim + 1, height_ratios=[1] * (ndim + 1))

    for i, lbl in enumerate(labels):
        axes[i].plot(steps, samples[:, i], lw=LW_FINE, alpha=ALPHA_STANDARD, color="C0")
        axes[i].set_ylabel(lbl)

    axes[-1].plot(steps, log_probs, lw=LW_FINE, alpha=ALPHA_STANDARD, color="C0")
...
def plot_corner(result):
    ...
    fig = corner.corner(
        ...
        color="C0",
        fig=plt.figure(figsize=(0.8 * TEXTWIDTH_IN, 0.8 * TEXTWIDTH_IN)),
    )
    return fig
...
def plot_posterior_predictive(result, *, color="C0", data_label="Data", ax=None):
    ...
    pool = samples[::step]
    if len(pool) > 400:
        pool = pool[rng.choice(len(pool), size=400, replace=False)]
    ...
    ax.fill_between(x_grid, q025, q975, color=color, alpha=ALPHA_EXTRA_LIGHT,
                    lw=LW_NONE, zorder=3, label=r"95\% predictive")
    ax.fill_between(x_grid, q16, q84, color=color, alpha=ALPHA_FAINT,
                    lw=LW_NONE, zorder=4, label=r"68\% predictive")
    ax.plot(x_grid, median_line, color=color, lw=LW_STANDARD, zorder=5,
