...
Provides survey-agnostic model builders that return standard
``torch.nn.Module`` objects ready for use with ``train_cnn``.
"""
...
def build_resnet18(n_labels: int, input_size: int) -> nn.Module:
    ...
    Adapts the standard torchvision ResNet-18 for arbitrary input sizes
    and multi-label float output (sigmoid activation, no softmax).

    For small inputs (< 64 px), the first 7x7 conv is replaced with a
    3x3 conv (stride 1, no initial max-pool) to avoid collapsing spatial
    dimensions too early.

    Parameters
    ----------
    n_labels : int
        Number of output labels (e.g. 37 for Galaxy Zoo 2).
    input_size : int
    ...
    -------
    nn.Module
        Modified ResNet-18 with sigmoid output.
    """
...
class _CustomCNN(nn.Module):
    ...
    Architecture:  [Conv2d -> ReLU -> Pool] x N_conv
                -> Flatten
