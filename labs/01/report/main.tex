...
\begin{figure*}[t]
  ...
  \begin{tikzpicture}[
    ...
    \node[pipebox, fill=red!20] (quality_cuts) at (4,-4.35) {Quality Cuts\\(130,877)};
...
To note, PL relations have wavelength dependence whereby slopes are steeper and scatter is smaller in the infrared. This is because dust extinction decreases with wavelength ($A_\lambda \propto \lambda^{-\alpha}$, $\alpha \sim 1.7$), so infrared PL relations are less sensitive to residual extinction in the calibration sample. WISE band $W2$ (4.6 $\mu$m) offers the additional advantage that the $R_{W2}$ extinction coefficient is nearly negligible \citep{KleinBloom2014}.
...
\begin{equation}
  \mathbf{X} = \begin{pmatrix} \mathbf{x}_1 \\ \mathbf{x}_2 \\ \vdots \\ \mathbf{x}_N \end{pmatrix}, \qquad
  \boldsymbol{\beta} = \begin{pmatrix} a_0 \\ a_1 \\ b_1 \\ \vdots \\ a_K \\ b_K \end{pmatrix}.
\end{equation}
...
Throughout this section we use an RRab sdtar Gaia DR3 source \texttt{4659759557323962752} as a running example to illustrate each analysis step.
...
Figure~\ref{fig:lc_raw_phased} shows raw and phase-folded light curves, and Figure~\ref{fig:periodogram} shows the corresponding LS periodogram.

\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_lc_raw_phased.pdf}
  \caption{Example Gaia DR3 $G$-band light curves. \textit{Top:} Raw time series with photometric uncertainties. \textit{Bottom:} Phase-folded light curve using the LS-recovered period, showing the characteristic sawtooth morphology of fundamental-mode pulsation.}
...
\begin{figure}[H]
  ...
  \caption{Example Lomb-Scargle periodogram. The vertical dashed line marks the peak period $P_{\rm LS}$ adopted for subsequent Fourier fitting.}
...
\begin{figure}[H]
  ...
  \caption{$G$-band period--absolute magnitude plot color-coded by posterior inlier probability. Circles show retained stars ($p_{\rm in} \geq 0.95$); crosses mark the flagged outliers ($p_{\rm in} < 0.95$) removed from the calibration sample.}
  \label{fig:pl_inlier}
\end{figure}
...
\begin{figure*}[b]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_clean_period_abs_mag.pdf}
  \caption{$G$-band period--absolute magnitude diagram for the final cleaned calibration sample ($N=884$) after mixture contamination model rejection ($p_{\rm in} \geq 0.95$). Points are colored by pulsation subclass (RRab, RRc, RRd). Error bars show combined photometric and parallax uncertainties.}
  \label{fig:clean_period_abs_mag}
\end{figure*}
...
% \textbf{M-H sampler validation.} Before applying any of the three methods to the calibration data, we validated the hand-written M-H implementation on an analytically tractable toy problem: $x_{\rm obs} = 1.0$ observed with $\sigma_{\rm obs} = 0.1$, so the exact posterior is $\mu \mid x \sim \mathcal{N}(1.0,\,0.1^2)$. Table~\ref{tab:mh_proposal_scan} reports the proposal-width scan; width $\sigma_{\rm prop} = 0.20$ gives $\approx$50\% acceptance---a commonly cited target for pedagogical 1-D examples, though not a universal optimum---and is selected for the final chain. After 10,000 steps (1,000 discarded as burn-in) the chain recovers $\hat{\mu} = 1.0005$ and $\hat{\sigma} = 0.1029$, matching the analytic values $\mu = 1.0000$ and $\sigma = 0.1000$ to better than 0.5\% (Figure~\ref{fig:mh_validation_posterior}).

% \begin{deluxetable}{cc}
% \tablewidth{0pt}
% \tablecaption{M-H Proposal-Width Scan on 1-D Gaussian Toy Problem\label{tab:mh_proposal_scan}}
% \tablecolumns{2}
% \tablehead{
%   \colhead{$\sigma_{\rm prop}$} & \colhead{Acceptance rate}
% }
% \startdata
% 0.05 & 0.839 \\
% 0.10 & 0.719 \\
...
% \begin{figure}[H]
%   \centering
%   \includegraphics[width=\columnwidth]{figures/fig_mh_validation_posterior.pdf}
%   \caption{Validation of the custom Metropolis-Hastings sampler on an analytically known 1-D posterior $\mu \mid x \sim \mathcal{N}(1.0,\,0.1^2)$. Blue histogram shows post-burn samples; the orange curve is the analytic Gaussian. The recovered mean and standard deviation (1.0005, 0.1029) match analytic values (1.0000, 0.1000) to better than 0.5\%.}
%   \label{fig:mh_validation_posterior}
% \end{figure}
...
\textbf{Method 3 (Native PyMC):} A fully probabilistic PyMC model with Gaussian likelihoods and HalfNormal priors, using the default NUTS sampler (1,500 tune, 3,000 draws). 

\begin{figure*}[b]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/fig_methods_corner_rrab.pdf}
  \caption{Overlay corner plot comparing all three MCMC methods for \textbf{RRab}: M-H (red), NUTS+Potential (brown), native PyMC (blue). All three converge to consistent posteriors, validating our implementation.}
  \label{fig:methods_corner_rrab}
\end{figure*}

Figures~\ref{fig:methods_corner_rrab} and \ref{fig:methods_corner_rrc} overlay all three methods for direct comparison of the RRab and RRc posteriors respectively. All methods converge to consistent posterior means, but NUTS-based methods achieve much lower autocorrelation.

% \begin{table}[ht!]
% \centering
% \caption{MCMC Method Comparison for RRab G-band PL Fit\label{tab:mcmc_compare}}
% \begin{tabular}{lcccc}
% \hline\hline
% Method & Accept.\ rate & $a$ & $b$ & $\sigma_{\rm scatter}$ \\
% \hline
...
\end{align}
Rather than overplotting individual posterior draws which can obscure the actual uncertainty structure by cluttering the figure, we display the posterior median relation together with 68\% and 95\% posterior predictive envelopes computed from the full post burn sample. This representation makes the shape and width of the uncertainty region immediately legible. 
The posterior predictive envelopes (shown in Figure~\ref{fig:optical_ir_comparison}) comfortably bracket the data with no systematic residual trends. The envelope width is approximately uniform across the full period range of the sample calibration stars the slope, and is tightly constrained ($\sigma_a \approx 0.13$). This is smaller than the intrinsic astrophysical scatter $\sigma_{\rm scatter} \approx 0.18$~mag, which dominates the envelope width throughout. The roughly constant band width therefore suggests that the uncertainty is set by possible stellar physics effects (metallicity spread, optical depth) rather than by parameter estimation uncertainty. 

\begin{figure*}[t]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/fig_methods_corner_rrc.pdf}
  \caption{Overlay corner plot comparing all three MCMC methods for \textbf{RRc}: M-H (purple), NUTS+Potential (pink), native PyMC (orange). The RRc posteriors are broader than RRab (fewer stars), but all three methods converge to consistent posteriors.}
...
\begin{align}
  a_{\rm RRc} &= -2.2169^{+0.1779}_{-0.1731}, \\
  b_{\rm RRc} &= +0.5757 \pm 0.0108, \\
  \sigma_{\rm scatter,RRc} &= 0.1669 \pm 0.0093.
\end{align}
...
%%% ---------------------------------------------------------------
\section{WISE W2 Cross-Match}
\label{sec:wise}
%%% ---------------------------------------------------------------

\subsection{Cross-Matching and Quality Cuts}

WISE $W2$ photometry was retrieved for calibration sample stars via the Gaia DR3\newline \texttt{allwise\_best\_neighbour} table (Section~\ref{sec:data}). The cross-match proceeds in stages; first, the full Gaia DR3 $\times$ AllWISE join yields 271,779 rows; restricting to our clean 884-star calibration sample gives 875 matched stars; after applying conservative photometric quality flags (\texttt{w2snr} $> 5$, no contamination or upper-limit flags), 839 stars remain (527 RRab + 304 RRc + 8 RRd). The 8 RRd stars are excluded from the PL fitting, leaving 831 stars used for the infrared analysis. The $\sim$5\% cross-match loss (884 $\to$ 839) is consistent with the expected AllWISE completeness at these faint magnitudes. The sky distribution and period--absolute-magnitude cuts applied here mirror the G-band sample-selection procedure described in Section~\ref{sec:calib} (see Figures~\ref{fig:calibration_sky_c12} and \ref{fig:pl_stages}).
...
Figure~\ref{fig:optical_ir_comparison} shows the posterior median and 68\%/95\% predictive envelopes over the RRab and RRc $W2$ data alongside the optical fits, and Table~\ref{tab:pl_summary} summarizes the results. The slopes are significantly steeper than in $G$:
\begin{align}
  a_{W2,\rm RRab} &= -2.9845 \pm 0.1189, \\
  a_{W2,\rm RRc}  &= -3.2460 \pm 0.1555.
\end{align}

\begin{figure*}[p]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_optical_ir_comparison.pdf}
  \caption{Side-by-side comparison of the Gaia $G$-band (blue) and WISE $W2$ (orange) period-luminosity relations for \textbf{RRab} (\textit{top}) and \textbf{RRc} (\textit{bottom}). Shaded bands show 68\% posterior predictive envelopes. The steeper $W2$ slope ($\Delta a_{\rm RRab} = -0.75 \pm 0.17$, $\Delta a_{\rm RRc} = -1.03 \pm 0.24$) is visible as a more pronounced tilt in the orange fit lines relative to the blue. The tighter $W2$ scatter also reflects reduced extinction sensitivity at mid-infrared wavelengths.}
...
The infrared slope is steeper by $\Delta a_{\rm RRab} = -0.75 \pm 0.17$ and $\Delta a_{\rm RRc} = -1.03 \pm 0.24$. This steepening is a well-known property of RR Lyrae PL relations where at optical wavelengths, the luminosity-temperature-radius coupling partially cancels the period-luminosity correlation; in the infrared regime, the relationship becomes more purely between period and bolometric luminosity \citep{CatelanSmith2015}. Figure~\ref{fig:optical_ir_comparison} shows both relations side by side for each subclass, making the slope difference directly visible.

\begin{table*}[ht!]
\centering
\caption{Period-Luminosity Relation Summary\label{tab:pl_summary}}
\begin{tabular}{llccc}
\hline\hline
Band & Type & $a$ & $b$ & $\sigma_{\rm int}$ \\
\hline
$G$ (this work) & RRab (560) & $-2.2316\pm0.1283$ & $0.6531\pm0.0085$ & $0.1762\pm0.0077$ \\
$G$ (this work) & RRc  (316) & $-2.2169^{+0.1779}_{-0.1731}$ & $0.5757\pm0.0108$ & $0.1669\pm0.0093$ \\
$W2$ (this work) & RRab (527) & $-2.9845\pm0.1189$ & $\cdots$ & $0.1475\pm0.0072$ \\
$W2$ (this work) & RRc  (305) & $-3.2460\pm0.1555$ & $\cdots$ & $0.1335\pm0.0087$ \\
...
\begin{figure*}[b!]
  ...
  \caption{Joint period-color posterior predictive comparison for RRab (blue) and RRc (orange) in the same coordinate system. The RRc slope ($a_{c,\rm RRc} = +0.48 \pm 0.05$) is roughly twice as steep as the RRab slope ($a_{c,\rm RRab} = +0.25 \pm 0.04$), confirming stronger temperature-period coupling in first-overtone pulsators.}
...
Figure~\ref{fig:period_color} shows the posterior predictive checks for both subclasses, making the factor-of-two slope difference directly visible. The tight scatter ($\sigma_c \approx 0.05$ mag) suggests that RR Lyrae intrinsic colors are well-constrained by period alone.
...
\begin{align}
  E(G_{\rm BP} - G_{\rm RP}) &= (G_{\rm BP} - G_{\rm RP})_{\rm obs} \notag \\
                              &\quad - a_c\,(\log P - \langle\log P\rangle_{\rm class}) - b_c,
\end{align}
...
\subsection{Comparison to Gaia DR3 \texttt{g\_absorption}}

As a sanity check, we compared our empirical $A_G$ estimates to the Gaia DR3 \texttt{g\_absorption} field, which uses the Gaia photometric solution to estimate extinction. Figure~\ref{fig:extinction_comparison} shows this comparison where we find that our median offset is 0.114 mag lower than \texttt{g\_absorption}.

\begin{figure}[H]
  \centering
  \includegraphics[width=\columnwidth]{figures/fig_extinction_comparison.pdf}
  \caption{Empirical $A_G$ (this work) vs.\ Gaia DR3 \texttt{g\_absorption} for 109,325 RRab stars with finite Gaia extinction estimates. The dashed line shows the 1:1 relation.}
...
\begin{enumerate}
  ...
  \item $\texttt{phot\_rp\_mean\_flux\_over\_error} > 5$ (good RP photometry): removes 1,474 stars (0.6\%)
  ...
  \item Uncertainty on color excess $\sigma_E \leq 0.15$ mag: removes 8,325 stars (3.3\%)
  \item $0 \leq E(G_{\rm BP}-G_{\rm RP}) \leq 10$ mag (exclude unphysical negative reddening): removes 38,582 stars (15.2\%), primarily low-latitude stars with negative empirical reddening. The upper bound sanity cap of 10 mag affects no stars passing the preceding quality cuts
\end{enumerate}

% Figure~\ref{fig:quality_diagnostics} shows the two photometric diagnostics that motivate cuts~(3) and~(4): the $\sigma_E$ histogram with the 0.15 mag threshold and the BP/RP excess factor scatter plot with the Gaia magnitude-dependent envelope. Figure~\ref{fig:reddening_distribution} shows the $E(G_{\rm BP}-G_{\rm RP})$ distribution in the stage-5 sample (after $\sigma_E$ cut), illustrating the negative tail and upper tail that motivate cut~(5).

% \begin{figure*}[t]
%   \centering
%   \includegraphics[width=\textwidth]{figures/fig_quality_diagnostics.pdf}
%   \caption{\textit{Left:} Distribution of propagated reddening uncertainty $\sigma_E$ for the full catalog. The dashed red line marks the $\sigma_E = 0.15$ mag cut threshold; stars to the right are removed by cut~(4). \textit{Right:} BP/RP photometric excess factor vs. $G_{\rm BP}-G_{\rm RP}$ color. Grey points lie within the Gaia magnitude-dependent envelope (blue dashed curves); red points fall outside and are removed by cut~(3). Concentration at redder colors reflects blended sources in high-extinction regions.}
%   \label{fig:quality_diagnostics}
% \end{figure*}

% \begin{figure}[H]
%   \centering
%   \includegraphics[width=\columnwidth]{figures/fig_reddening_distribution.pdf}
%   \caption{Distribution of $E(G_{\rm BP}-G_{\rm RP})$ in the stage-5 sample (after the $\sigma_E$ cut). The bulk of the distribution is positive and moderate, as expected. The dashed lines mark the $E = 0.0$ and $E = 10.0$ mag thresholds applied as cut~(5); stars outside this range are removed as physically unreasonable.}
%   \label{fig:reddening_distribution}
% \end{figure}

After these cuts, 130,877 stars (48.5\% of the initial catalog) remained for the sky map. The removed stars are not random but concentrated toward the Galactic plane and other crowded or high-extinction regions.

% \begin{figure*}[t]
%   \centering
%   \includegraphics[width=\textwidth]{figures/fig_removed_stars_map.pdf}
%   \caption{Aitoff sky map of the 138,826 stars removed by the five quality cuts. Points are colored by $E(G_{\rm BP}-G_{\rm RP})$ on the same scale as Figure~\ref{fig:reddening_map}. Removals are concentrated toward the Galactic plane ($|b| \lesssim 20^\circ$), the Galactic center, and high-density satellite fields (LMC/SMC region), where Gaia BP/RP photometry is most susceptible to blending. High-latitude rejections ($|b| > 30^\circ$) are sparse and reflect individual photometric outliers rather than coherent dust structure.}
%   \label{fig:removed_stars_map}
% \end{figure*}

\subsection{All-Sky Reddening Map}

Figure~\ref{fig:reddening_map} shows the all-sky reddening map in Aitoff projection and figure~\ref{fig:sfd_map} shows the SFD reddening map.
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\dimexpr\paperheight-1.5in\relax]{figures/fig_reddening_map.pdf}
  \caption{All-sky reddening map from 130,877 RR Lyrae stars (88,028 RRab + 42,849 RRc) in Aitoff (equal-area) projection. Color indicates $E(G_{\rm BP}-G_{\rm RP})$ in magnitudes. The Galactic plane shows high reddening; the Galactic center region (right) has the highest extinction. High-latitude regions show low, patchy reddening from nearby dust clouds; the Large and Small Magellanic Clouds appear clearly as overdensities. The non-uniform sky coverage reflects the non-uniform distribution of RR Lyrae stars.}
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\dimexpr\paperheight-1.5in\relax]{figures/fig_sfd_map.pdf}
  \caption{SFD $E(B-V)$ \citep{SFD1998} sampled at the sky positions of the 130,877 cleaned RR Lyrae stars in Aitoff (equal-area) projection. The map traces the total dust column integrated to infinity along each sightline; compare with Figure~\ref{fig:reddening_map}, which shows only the foreground dust out to each star's heliocentric distance. The Galactic plane and center show markedly higher $E(B-V)$ in SFD than in the empirical RR Lyrae map which provides direct evidence of the 3D~vs.~2D effect explored quantitatively in Section~\ref{sec:regime}.}
  \label{fig:sfd_map}
\end{figure}
\end{landscape}
\restoregeometry
...
% \begin{figure}[p]
%   \centering
%   \includegraphics[width=\textheight, height=\textwidth, keepaspectratio]{figures/fig_sfd_comparison.pdf}
%   \caption{Hexbin density plot of RR Lyrae-derived $E(G_{\rm BP}-G_{\rm RP})$ (this work) vs.\ SFD $E(B-V)$ for the 130,877 cleaned map stars. Color encodes $\log_{10}$(count per bin); the white/red curve is the binned median trend. No unit conversion is applied; the Spearman rank correlation $\rho = 0.913$ is computed directly between the two quantities.}
...
\begin{table}[ht!]
\centering
\caption{Pearson $R^2$ with SFD by Galactic Latitude ($10^\circ$ bins; $|b|>60^\circ$ split into finer bins)\label{tab:sfd_latitudes}}
\begin{tabular}{lcc}
\hline\hline
Latitude range & $N_{\rm stars}$ & $R^2$ \\
\hline
$|b| < 10^\circ$               & 54,295  & 0.3483 \\
$10^\circ < |b| < 20^\circ$   & 33,357  & 0.8225 \\
$20^\circ < |b| < 30^\circ$   & 17,360  & 0.7693 \\
$30^\circ < |b| < 40^\circ$   & 18,142  & 0.0548 \\
$40^\circ < |b| < 50^\circ$   & 5,055   & 0.1656 \\
$50^\circ < |b| < 60^\circ$   & 1,435   & 0.0335 \\
$60^\circ < |b| < 70^\circ$   & 676     & 0.0290 \\
$70^\circ < |b| < 80^\circ$   & 407     & 0.0156 \\
$|b| > 80^\circ$               & 150     & 0.0000 \\
\hline
Overall                        & 130,877 & 0.459  \\
\hline\hline
\end{tabular}
\end{table}
...
\begin{figure*}[b!]
  ...
  \textit{Left:} Similar-scale regime ($E(B-V)_{\rm SFD} \leq 2$ mag, $N = 127{,}893$ stars). The density hexbin shows a tight linear trend; the dashed orange line is a linear fit with slope $1.11$, below the expected Gaia-band conversion factor $R_{BP-RP} \approx 1.3$.
  \textit{Right:} Large-SFD regime ($E(B-V)_{\rm SFD} > 10$ mag, $N = 74$ stars). The empirical $E(G_{\rm BP}-G_{\rm RP})$ saturates far below the SFD column despite the physical ceiling at 10.0 mag being well above the observed values. The decoupling illustrates the 3D~vs.~2D distinction between a distance-limited stellar measurement and a total-column dust map.}
  \label{fig:regime_decomposition}
\end{figure*}

The overall $R^2 = 0.459$ suggests the period-color approach for dust tracing is effective. Interestingly, $R^2$ peaks in the intermediate-latitude bins ($R^2 = 0.8225$ at $10^\circ$--$20^\circ$ and $0.7693$ at $20^\circ$--$30^\circ$) rather than at the lowest latitudes ($R^2 = 0.3483$ at $|b| < 10^\circ$), and drops sharply at high latitudes. The finer breakdown at high latitudes (Table~\ref{tab:sfd_latitudes}) shows that $R^2$ falls below 0.01 between $60^\circ$ and $80^\circ$ and shows no meaningful signal at the poles (150 stars at $|b| > 80^\circ$). This latitude dependence is physically expected: near the plane, the dust column is large (up to $E(B\!-\!V) \sim 5$ mag), providing a large dynamic range that allows both methods to correlate well. At high latitudes, the dust column is small ($E(B\!-\!V) \lesssim 0.05$ mag) and both measurements are noise-dominated.

\subsection{SFD Regime Decomposition Comparison}
\label{sec:regime}
...
\begin{enumerate}
  \item \textbf{Similar-scale} ($E(B-V)_{\rm SFD} \leq 2$ mag, $N = 127{,}893$ stars): SFD and the empirical RR Lyrae color excess are in the same range.
  \item \textbf{Large-SFD} ($E(B-V)_{\rm SFD} > 10$ mag, $N = 74$ stars): these are the most heavily obscured Galactic plane and inner-Galaxy sightlines.
\end{enumerate}

Figure~\ref{fig:regime_decomposition} shows a two-panel summary. In the similar-scale panel (left), a hexbin density plot combined with a linear fit reveals a slope of $1.11$, somewhat below the expected Gaia-band reddening conversion factor $R_{BP-RP} \equiv E(G_{\rm BP}-G_{\rm RP})/E(B-V) \approx 1.3$ for a standard $R_V = 3.1$ extinction law \citep{Fitzpatrick1999, WangChen2019}. The deviation from unity is consistent with the 3D vs.\ 2D effect where in this moderate-reddening subsample, RR Lyrae stars at finite distances measure only a foreground fraction of the SFD column. Furthermore, the Pearson $R^2$ within this subsample is $0.878$, and the binned median trend follows the linear fit closely, confirming that the empirical period-color calibration recovers a consistent reddening signal relative to SFD across the entire moderate-extinction range.
...
Table~\ref{tab:pl_summary} summarizes our PL relations. The $G$-band slopes for RRab and RRc are consistent within $2\sigma$, though RRab are slightly steeper. In optical bands, RRab and RRc occupy different parts of the instability strip and may follow subtly different PL relations due to their different mass-luminosity relationships. 

The intrinsic scatter $\sigma_{\rm scatter} \approx 0.17$--0.18 mag in $G$ could be dominated by the unmodelled metallicity spread, $Z_\lambda [\mathrm{Fe/H}]$.

Our $W2$ RRab slope ($-2.98 \pm 0.12$) lies $\sim$2.5$\sigma$ from the \citet{KleinBloom2014} value ($a = -2.39 \pm 0.20$), possibly reflecting different sample selections or parallax zero-points. It agrees within $1\sigma$ with the pure PL slope reported by \citet{Dambis2014} ($a = -2.94 \pm 0.15$). The small differences may reflect (i) different metallicity distributions between samples, (ii) different parallax zero-point conventions (Gaia EDR3 vs. HST or other calibrators), or (iii) different reference period choices. For RRc stars, \citet{KleinBloom2014} report $a_{W2,\rm RRc} = -1.70 \pm 0.62$, which is substantially shallower than our value of $-3.23 \pm 0.16$. The large uncertainty on the \citet{KleinBloom2014} RRc slope ($\pm 0.62$) reflects their much smaller sample size. Compared to \citet{KleinBloom2014}, our RRc slope deviates at the $\sim$2.5$\sigma$ level.
...
Literature values are listed in Table~\ref{tab:pl_summary} for direct numerical comparison but are \textit{not} overlaid on the posterior predictive check figures (Figures~\ref{fig:optical_ir_comparison} and \ref{fig:period_color}). Those figures display our model evaluated over our own calibration sample; overlaying a literature relation calibrated on a different dataset, with different parallax zero-points, and metallicity distributions, would conflate the intrinsic scatter of our data with the systematic offset between datasets and produce a misleading visual comparison.

\subsection{Wavelength Dependence}

The $W2$ slopes are steeper by $\Delta a \approx -0.75$ (RRab) and $\approx -1.03$ (RRc) compared to $G$. This confirms the well-established wavelength dependence of RR Lyrae PL relations. The reduced scatter in $W2$ ($\sigma_{\rm scatter} \approx 0.14$ mag) has two contributions: (i) the metallicity effect on luminosity is smaller in the infrared \citep{CatelanSmith2015}, and (ii) the $W2$ observations are unaffected by residual extinction in our sample, which at $|b|>30^\circ$ amounts to $A_G \approx 0.0$--0.2 mag.

\subsection{Dust Map Quality and Limitations}

Our 130,877-star reddening map is a large RR Lyrae-based dust maps compiled from Gaia data. Primarily, it is limited by sampling inhomogeity and distance-limited coverage of RR Lyrae stars observable. RR Lyrae are old ($>10$ Gyr) metal-poor stars that trace the Galactic halo and bulge. Their sky distribution is thus non-uniform in a physically convenient sense since they concentrate toward the Galactic center (bulge overdensity) and are distributed throughout the halo, but are severely under-represented in the Galactic plane ($|b| \lesssim 10^\circ$) because the dense dust column makes RR Lyrae in the plane observationally inaccessible in the optical $G$ band. The outer disk and spiral arms, which host young stellar populations but very few old metal-poor halo stars, and thus are also poorly sampled. This contrasts with dust, which follows the young stellar population and interstellar gas more closely.

Additionally, most stars in our sample from Gaia with good astrometric data are within $\sim$10 kpc. The SFD map, derived from far-infrared emission, includes all Galactic dust to infinity. Our map will therefore underestimate extinction toward the Galactic center and beyond the outer disk.

Lastly, we also note that the maps look patchy with missing gaps of data. With 130,877 stars over the full sky, the mean angular separation between stars is $\sim 0.3^\circ$. The effective angular resolution of the map is thus limited to $\sim$0.3--0.5 degrees, comparable to the SFD resolution of $\sim 6'$ but with much noisier individual measurements.
...
We have constructed an empirical all-sky dust reddening map from Gaia DR3 RR Lyrae stars using a pipeline that proceeds from raw light curve data through period recovery, Fourier modeling, calibration, and Bayesian inference to a 130,877-star sky map.

Period recovery from Lomb-Scargle periodograms agrees with Gaia catalog values to within 1\% for 89 of 100 test stars. All 11 outlying cases arise because $P_{\rm LS}$ latches onto the first-overtone period \texttt{p1o} rather than the fundamental \texttt{pf}, identifying these stars as RRd double-mode pulsators. Blazhko-effect modulations, though visible in some phase-folded light curves, carry negligible LS power and do not affect period recovery.

Mean magnitudes are estimated by integrating cross-validated Fourier series fits (optimal order $K \approx 5$--7) in flux space. This reduces the RMS from 0.0282 to 0.0147 mag relative to na\"{\i}ve epoch-averaging, owing to the non-linear magnitude--flux relation and the asymmetric sawtooth shape of RRab light curves.

The calibration sample was assembled from 1050 nearby ($d < 4$ kpc), high-latitude ($|b| > 30^\circ$) stars and cleaned through Lindegren C1/C2 astrometric quality cuts and a two-component mixture contamination model, arriving at 884 stars (560 RRab + 316 RRc + 8 RRd). $G$-band period-luminosity relations fit by Bayesian NUTS sampling give $a_{\rm RRab} = -2.23\pm0.13$, $b_{\rm RRab} = 0.653\pm0.009$ and $a_{\rm RRc} = -2.22\pm0.18$, $b_{\rm RRc} = 0.576\pm0.011$, with three independent MCMC approaches (Metropolis-Hastings, NUTS with a custom potential, native PyMC) yielding mutually consistent posteriors.

Extending to WISE $W2$ photometry, we find slopes steeper by $\Delta a \approx -0.75$ (RRab) and $\approx -1.03$ (RRc) relative to $G$, with reduced intrinsic scatter ($\sigma_{\rm scatter} \approx 0.14$ mag), consistent with the well-established wavelength dependence of RR Lyrae period-luminosity relations and the near-zero $W2$ extinction coefficient. The RRab $W2$ slope ($-2.98\pm0.12$) agrees within $1\sigma$ with \citet{Dambis2014} ($a = -2.94\pm0.15$). Period-color relations in $G_{\rm BP}-G_{\rm RP}$ are tight ($\sigma_c \approx 0.05$ mag) with slopes $a_{c,\rm RRab} = +0.25\pm0.04$ and $a_{c,\rm RRc} = +0.48\pm0.05$.

Applying the period-color calibration to the full 269,772-star catalog and retaining 130,877 stars (88,028 RRab + 42,849 RRc) after quality filtering yields an all-sky reddening map with Pearson $R^2 = 0.459$ against the SFD map overall and $R^2 = 0.878$ in the similar-scale regime ($E(B-V)_{\rm SFD} \leq 2$ mag, $N = 127{,}893$). A regime decomposition reveals a linear fit slope of 1.11 in that subsample, below the expected $R_{BP-RP} \approx 1.3$, consistent with RR Lyrae at finite distances measuring only a foreground fraction of the total SFD column. In the large-SFD regime ($E(B-V)_{\rm SFD} > 10$ mag, $N = 74$), empirical reddening values saturate well below the SFD column, directly illustrating the 3D vs.\ 2D distinction inherent to stellar dust tracers.
...
\begin{figure}[p]
  \centering
  \includegraphics[width=\dimexpr\paperheight-1.5in\relax]{figures/fig_reddening_map_dark.pdf}
  \caption{A \textit{cooler} looking all-sky reddening map rendered on a black background using the \texttt{inferno} colorscale, mapping zero reddening to black and increasing dust columns to progressively brighter orange and yellow. Same 130,877 stars as Figure~\ref{fig:reddening_map}. The dark background suppresses empty sky and makes the filamentary structure of nearby dust clouds and the Galactic warp visually prominent.}
  \label{fig:reddening_map_dark}
\end{figure}
\end{landscape}
\restoregeometry
