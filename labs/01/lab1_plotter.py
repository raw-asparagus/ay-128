...
def _plot_period_values(source) -> np.ndarray:
    return np.asarray(_as_table(source)["rrlyrae_representative_period"], dtype=float)
...
def plot_raw_phase_folded_lightcurve(source_id: int, data: Table):
    ...
    if len(data) == 0:
        raise ValueError("No light-curve rows available for plotting.")
    classification = str(data["best_classification"][0])
    period = float(np.asarray(data["rrlyrae_representative_period"], dtype=float)[0])
